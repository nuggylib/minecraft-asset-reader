...
import { CONTENTFUL_OAUTH_APPS_ADDRESS, CREATE_CONTENTFUL_OAUTH_ADDRESS } from "./constants";
...
export const createOrUseExistingPrompt = () => inquirer.prompt([
    {
        type: 'list',
        name: 'choice',
...
.then(answers => {
    ...
    switch(answers.choice) {
        case CHOICES.CREATE: {
            console.log(`Navigate to ${CREATE_CONTENTFUL_OAUTH_ADDRESS} to create OAuth app credentials (you can delete this app later after importing data)`)
        ...
        case CHOICES.USE_EXISTING: {
            console.log(`Navigate to ${CONTENTFUL_OAUTH_APPS_ADDRESS} and select the app to use (note that the redirect URL must point to '${redirect}'; you may need to update the app before continuing)`)

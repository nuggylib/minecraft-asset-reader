...
export function loadTexturesFromBlockstateVariants(args: { namespace: string, blockName: string, assetsPath: string, rawData: RawAssetData }): string[] {
    let texturesForBlockModelFile = [] as string[]
    // The textures referenced in the model.json file - can be one or many
    const variants = args.rawData[args.namespace].blockstates[args.blockName].variants
    ...
    Object.keys(variants!).forEach(key => {
        ...
        try {
            const modelData = readFileSync(`${args.assetsPath}/${args.namespace}/models/block/${modelName}.json`, 'utf8')
            ...
            console.error(`Error evaluating key '${key}' for block '${args.blockName}': ` + e.message)
        }
 ...
 */
export function loadTexturesFromTextureField(args: { namespace: string, blockModelFileName: string, rawData: RawAssetData }): string[] {
    let texturesForBlockModelFile = [] as string[]
    // The textures referenced in the model.json file - can be one or many
    const blockTextures = args.rawData[args.namespace].model.block[args.blockModelFileName].textures
 ...
 */
export function loadTexturesFromElements({ namespace, blockModelFileName }: { namespace: string, blockModelFileName: string }): string[] {
    let texturesForBlockModelFile = [] as string[]

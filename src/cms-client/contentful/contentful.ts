...
export async function createContentType({
  ...
  return fetch(endpoint, {
    ...
    .then((res) => res.json())
    .catch((err) =>
      console.error(`Unable to add content type '${typeName}': ${err.message}`)
    )
}
...
export async function createSpace({
  ...
  return fetch(endpoint, {
    ...
    .then((res) => res.json())
    .catch((err) =>
      console.error(`Unable to add space '${spaceName}': ${err.message}`)
    )
}
...
export async function publishContentType({
  ...
  return (
    fetch(endpoint, {
      ...
      .then((res) => res.json())
      .catch((err) =>
        console.error(`Unable to get environments: ${err.message}`)
      )
  )
}
 ...
 * @param param0
 */
export async function createContentTypeEntry({
  contentTypeId,
  fields,
}: {
  contentTypeId: string
